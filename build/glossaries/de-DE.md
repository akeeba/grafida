# Grafida — Glossar Deutsch (de-DE)

Verbindliche Übersetzungen der wichtigsten Begriffe für alle deutschen Texte in Grafida.
Eigennamen (Grafida, Joomla!, API, Markdown, HTML, TinyMCE) werden nicht übersetzt.
...
| save | Speichern | Imperativ als Schaltflächenbeschriftung. |
| cancel | Abbrechen | Imperativ als Schaltflächenbeschriftung. |
| delete | Löschen | Imperativ als Schaltflächenbeschriftung; endgültige Aktion. |
| edit | Bearbeiten | Imperativ als Schaltflächenbeschriftung. |
| title | Titel | Titel eines Artikels oder einer Website. |
| add | Hinzufügen | Imperativ, z. B. „Website hinzufügen". |
| import | Importieren | Dateiimport, z. B. Markdown-Import. |
| copy | Kopieren | In die Zwischenablage kopieren. |
...
| image path (predicted) | Bildpfad | Der Pfad, unter dem ein lokales Bild veröffentlicht wird; „Vorhersage", kein endgültiger Wert (gh-72). |
...
| server (AI provider) | Server | Serverinfrastruktur des KI-Anbieters; „Server" bleibt unverändert (Fremdwort). GRAFIDA_LBL_AI_STORE. |
| store (conversation, Responses API) | speichern | Serverseitiges Speichern einer Unterhaltung beim Anbieter, damit sie später fortgesetzt werden kann, statt den gesamten Verlauf erneut zu senden. GRAFIDA_LBL_AI_STORE. |
| resume window (days) | Tage (Fortsetzungsfrist) | Anzahl Tage, für die eine gespeicherte Unterhaltung als fortsetzbar gilt, bevor Grafida auf das erneute Senden des vollständigen Verlaufs zurückfällt. GRAFIDA_LBL_AI_STORE_DAYS. |
...
| created by alias (author) | Autor-Alias | The by-line shown instead of the real author's name (Joomla `created_by_alias`). A person's alias — NOT the URL slug above. GRAFIDA_LBL_CREATED_BY_ALIAS. |
