# Glossary — Spanish, Spain (es-ES)

Terms used consistently across the Grafida UI and its Spanish (Spain) translation.
Proper nouns kept as-is: Grafida, Joomla!, API, Markdown, HTML, TinyMCE.

| English term    | Spanish translation        | Notes                                                                                    |
|-----------------|----------------------------|------------------------------------------------------------------------------------------|
| access          | acceso                     | As in access level / permissions.                                                        |
| article         | artículo                   | Joomla! content item. Plural: artículos.                                                 |
| cancel          | Cancelar                   | Button label; infinitive used as imperative in Spanish UI conventions.                   |
...
| publish         | Publicar                   | Button label; infinitive. Use «publicar» (lowercase) in prose.                          |
| read more       | leer más                   | The Joomla! "Read more" break. Keep lowercase when used as a concept.                   |
| save            | Guardar                    | Button label; infinitive. Standard term in Spanish software UIs.                         |
| settings        | Ajustes                    | Application or item settings. Plural. Preferred over «Configuración» for UI labels.     |
| site            | sitio                      | A Joomla! site/installation. Short form; full form: «sitio web». Plural: sitios.        |
...
| retry | Reintentar | Botón que repite una solicitud fallida (gh-29). |
| offline / site unreachable | No se pudo contactar con el sitio | Fallo de transporte (DNS/conexión/tiempo de espera), distinto de un token de API rechazado. GRAFIDA_MSG_OFFLINE. |
| internet connection | conexión a Internet | La conexión de red del dispositivo, distinta de la «conexión» con la API del sitio. |
...
| image path (predicted) | ruta de la imagen | La ruta donde se publicará una imagen local; «previsión», no un valor definitivo (gh-72). |
...
| created by alias (author) | Alias del autor | The by-line shown instead of the real author's name (Joomla `created_by_alias`). A person's alias — NOT the URL slug above. GRAFIDA_LBL_CREATED_BY_ALIAS. |
| icon picker | selector de iconos | Searchable dropdown for the AI-tool icon. GRAFIDA_BTN_AI_TOOL_ICON_CHOOSE / GRAFIDA_PLACEHOLDER_AI_TOOL_ICON_SEARCH. «icono» per GRAFIDA_LBL_AI_TOOL_ICON. |
| list site articles | ver artículos del sitio | Empty-state action that switches to the Remote Articles tab. GRAFIDA_BTN_LIST_SITE_ARTICLES. |
| visit site | Visitar el sitio | Sidebar button under the site favicon; opens the site in the OS browser. GRAFIDA_BTN_OPEN_SITE. |
| meta description | Meta descripción | Joomla keeps the “Meta” prefix here (JFIELD_META_DESCRIPTION_LABEL), unlike keywords. GRAFIDA_LBL_METADESC. |
| keywords | Palabras clave | Joomla 4+ dropped the “Meta” prefix on this label (JFIELD_META_KEYWORDS_LABEL); Grafida follows suit. GRAFIDA_LBL_METAKEY. |
| unsupported fields | campos no compatibles | Custom-field types Grafida cannot edit; shown as a sidebar notice. GRAFIDA_MSG_UNSUPPORTED_FIELDS. |
...
| local media / local image | Medios locales / imagen local | An image stored in `media_blobs`, not yet published to the site (gh-36); tab label "Medios locales (aún no subidos)". |
| not yet published/uploaded | aún no subido/a / aún no publicado/a | State of a local image or article the site does not yet know about. |
| save to disk | Guardar en el disco | Writes a local media blob's bytes to disk via the native folder picker (Boson has no Save-As dialog). GRAFIDA_BTN_SAVE_TO_DISK. |
| used by (article) | Usada por «%s» | Shows which local article references a local image. GRAFIDA_LBL_MEDIA_USED_BY. |
| revision (of a local image) | (sin término propio) | The internal revision token in the local media URL (`rev` parameter); never appears in UI text. |
| site metadata | Metadatos del sitio | The locally cached categories, tags, access levels, languages and custom fields of a site (gh-42). GRAFIDA_LBL_METADATA_CACHE. |
...
| filesystem (Media Manager) | sistema de archivos | Un adaptador del gestor multimedia del sitio (p. ej. «local-images»). El plugin de Joomla! se llama «Sistema de archivos - Local». GRAFIDA_LBL_MEDIA_ADAPTER. |
| upload folder | Carpeta de destino | La carpeta, dentro del sistema de archivos elegido, donde se suben las imágenes de un artículo publicado (gh-57). GRAFIDA_LBL_MEDIA_FOLDER. |
| Unicode aliases (site setting) | alias Unicode | La opción «Alias Unicode» de la configuración global de Joomla!: decide si un alias conserva sus caracteres no ASCII. En Grafida es un ajuste de tres valores por sitio (Automático/Sí/No), porque solo un Super Usuario puede leerla (gh-61). GRAFIDA_LBL_UNICODE_ALIASES. |
| Super User | Super Usuario | El grupo de usuarios más alto de Joomla!; solo su token de API puede leer la configuración global. Aparece en textos de ayuda, sin clave propia. |
| publish anyway | Publicar de todos modos | Botón que publica pese a haber campos personalizados obligatorios que Grafida no puede editar, devolviendo sin cambios los valores leídos del sitio (gh-59). GRAFIDA_BTN_PUBLISH_ANYWAY. |
| publish blocked | Publicación bloqueada | Título del diálogo que detiene una publicación porque el sitio exige campos personalizados de un tipo no compatible. GRAFIDA_MSG_PUBLISH_BLOCKED_TITLE. |
| required (field) | obligatorio | Campo que Joomla! exige rellenar; sin valor, el sitio rechaza el artículo. |
| normalise (content) | normalizar | Quitar los caracteres invisibles del contenido generado por IA. GRAFIDA_LBL_CONTENT_NORMALISATION. |
| invisible marks | marcas invisibles | Término colectivo para los caracteres invisibles que se eliminan (ancho cero, direccionales, de etiqueta). GRAFIDA_OPT_CONTENT_NORMALISATION_INVISIBLE. |
| zero-width space | espacio de ancho cero | U+200B y afines; no ocupa espacio, pero viaja con el texto. |
| watermark | marca de agua | Marcado invisible incrustado a propósito en el texto de una IA. |
| screen reader | lector de pantalla | Software de lectura de pantalla; también anuncia los caracteres invisibles — la razón de accesibilidad de este ajuste. |
