<?php

/**
 * Grafida — Joomla content editing, untethered.
 *
 * @copyright Copyright (c) 2026 Nicholas K. Dionysopoulos
 * @license   GNU General Public License version 3, or later
 */

declare(strict_types=1);

namespace Grafida\I18n;

/**
 * The interface strings sent to the front-end at start-up.
 *
 * This is a pure i18n concern (the set of `language/en-GB/en-GB.ini` keys the
 * SPA needs translated), not controller state, so it lives here rather than
 * on a controller base class. Consumed by `BootstrapController::bootstrap()`
 * (the initial payload) and `SettingsController::setLanguage()` (re-sent
 * after switching language) via `LanguageService::strings(UiStrings::KEYS)`.
 */
final class UiStrings
{
    /** @var list<string> */
    public const KEYS = [
        'GRAFIDA_APP_TITLE', 'GRAFIDA_APP_SUBTITLE', 'GRAFIDA_NAV_SITES', 'GRAFIDA_NAV_ARTICLES', 'GRAFIDA_NAV_SETTINGS',
        'GRAFIDA_LBL_ARTICLE_PROPERTIES', 'GRAFIDA_BTN_TOGGLE_SIDEBAR', 'GRAFIDA_BTN_TOGGLE_PROPERTIES',
        'GRAFIDA_BTN_ADD_SITE', 'GRAFIDA_BTN_EDIT', 'GRAFIDA_BTN_SAVE', 'GRAFIDA_BTN_CANCEL',
        'GRAFIDA_BTN_DELETE', 'GRAFIDA_BTN_YES', 'GRAFIDA_BTN_NO', 'GRAFIDA_BTN_ON', 'GRAFIDA_BTN_OFF',
        'GRAFIDA_BTN_PUBLISH', 'GRAFIDA_BTN_PUBLISH_ANYWAY', 'GRAFIDA_BTN_NEW_ARTICLE', 'GRAFIDA_BTN_IMPORT_MD',
        'GRAFIDA_BTN_TEST_CONNECTION', 'GRAFIDA_BTN_INSERT_READMORE', 'GRAFIDA_BTN_COPY_HTML',
        'GRAFIDA_BTN_SAVE_AND_BACK', 'GRAFIDA_BTN_KEEP_EDITING', 'GRAFIDA_BTN_DISCARD_CHANGES',
        'GRAFIDA_BTN_DELETE_DRAFT', 'GRAFIDA_BTN_KEEP_DRAFT',
        'GRAFIDA_BTN_BACK', 'GRAFIDA_BTN_REFRESH', 'GRAFIDA_BTN_RELOAD_METADATA',
        'GRAFIDA_MSG_REFS_REFRESHED',
        'GRAFIDA_LBL_TITLE', 'GRAFIDA_LBL_ALIAS', 'GRAFIDA_PLACEHOLDER_ALIAS', 'GRAFIDA_BTN_REGENERATE_ALIAS',
        'GRAFIDA_LBL_URL', 'GRAFIDA_LBL_TOKEN', 'GRAFIDA_LBL_CATEGORY',
        'GRAFIDA_LBL_EDITOR_CSS_URL', 'GRAFIDA_MSG_EDITOR_CSS_URL_HINT',
        'GRAFIDA_LBL_MEDIA_ADAPTER', 'GRAFIDA_LBL_MEDIA_FOLDER',
        'GRAFIDA_OPT_MEDIA_ADAPTER_AUTO', 'GRAFIDA_MSG_MEDIA_UPLOAD_HINT',
        'GRAFIDA_LBL_UNICODE_ALIASES', 'GRAFIDA_MSG_UNICODE_ALIASES_HINT',
        'GRAFIDA_OPT_UNICODE_ALIASES_AUTO', 'GRAFIDA_OPT_UNICODE_ALIASES_YES',
        'GRAFIDA_OPT_UNICODE_ALIASES_NO',
        'GRAFIDA_LBL_TAGS', 'GRAFIDA_LBL_ACCESS', 'GRAFIDA_LBL_LANGUAGE', 'GRAFIDA_LBL_STATUS',
        'GRAFIDA_LBL_CREATED_BY_ALIAS',
        'GRAFIDA_LBL_METADESC', 'GRAFIDA_LBL_METAKEY', 'GRAFIDA_MSG_UNSUPPORTED_FIELDS',
        'GRAFIDA_MSG_SIDEBAR_FAILED',
        'GRAFIDA_LBL_SETTINGS', 'GRAFIDA_OPT_PUBLISHED', 'GRAFIDA_OPT_UNPUBLISHED',
        'GRAFIDA_OPT_ARCHIVED', 'GRAFIDA_OPT_TRASHED', 'GRAFIDA_OPT_NOT_LOADED',
        'GRAFIDA_LBL_UI_LANGUAGE', 'GRAFIDA_OPT_AUTO',
        'GRAFIDA_LBL_DISPLAY_MODE', 'GRAFIDA_OPT_DISPLAY_AUTO',
        'GRAFIDA_OPT_DISPLAY_LIGHT', 'GRAFIDA_OPT_DISPLAY_DARK',
        'GRAFIDA_LBL_STORAGE', 'GRAFIDA_LBL_DB_LOCATION', 'GRAFIDA_BTN_OPEN_FOLDER',
        'GRAFIDA_LBL_RESET_STORAGE', 'GRAFIDA_MSG_RESET_STORAGE_DESC', 'GRAFIDA_BTN_RESET_STORAGE',
        'GRAFIDA_MSG_RESET_STORAGE_CONFIRM', 'GRAFIDA_MSG_RESET_STORAGE_DONE',
        'GRAFIDA_MSG_CONNECTION_OK',
        'GRAFIDA_MSG_CONNECTION_FAIL', 'GRAFIDA_MSG_OFFLINE', 'GRAFIDA_BTN_RETRY', 'GRAFIDA_MSG_INSECURE_WARNING', 'GRAFIDA_MSG_PUBLISH_OK',
        'GRAFIDA_MSG_PUBLISH_BLOCKED_TITLE', 'GRAFIDA_MSG_PUBLISH_BLOCKED',
        'GRAFIDA_MSG_PUBLISH_BLOCKED_NO_VALUES', 'GRAFIDA_MSG_PUBLISH_FORCEABLE',
        'GRAFIDA_MSG_PUBLISH_FORCEABLE_WARNING', 'GRAFIDA_MSG_HTML_COPIED',
        'GRAFIDA_MSG_CLIPBOARD_FAILED', 'GRAFIDA_MSG_PUBLISH_REJECTED_TITLE',
        'GRAFIDA_MSG_PUBLISH_REJECTED', 'GRAFIDA_MSG_POST_PUBLISH_TITLE',
        'GRAFIDA_MSG_POST_PUBLISH_PROMPT', 'GRAFIDA_MSG_NO_SITES',
        'GRAFIDA_MSG_NO_SITES_SHORT', 'GRAFIDA_MSG_SAVED',
        'GRAFIDA_MSG_UNSAVED_TITLE', 'GRAFIDA_MSG_UNSAVED_CHANGES',
        'GRAFIDA_MSG_DELETE_DRAFT_TITLE', 'GRAFIDA_MSG_DELETE_DRAFT_CONFIRM',
        'GRAFIDA_MSG_DRAFT_DELETED', 'GRAFIDA_MSG_DELETE_SITE_CONFIRM',
        'GRAFIDA_MSG_UPDATE_AVAILABLE',
        'GRAFIDA_LBL_ABOUT', 'GRAFIDA_BTN_ABOUT', 'GRAFIDA_LBL_VERSION', 'GRAFIDA_LBL_LICENSE',
        'GRAFIDA_ABOUT_VIEW_LICENSE', 'GRAFIDA_BTN_CLOSE',
        'GRAFIDA_LBL_SITE', 'GRAFIDA_MSG_CHANGE_SITE_TITLE', 'GRAFIDA_MSG_CHANGE_SITE_CONFIRM',
        'GRAFIDA_LBL_IMAGES', 'GRAFIDA_LBL_INTRO_IMAGE', 'GRAFIDA_LBL_FULLTEXT_IMAGE',
        'GRAFIDA_LBL_IMAGE_URL', 'GRAFIDA_LBL_IMAGE_ALT', 'GRAFIDA_LBL_IMAGE_CAPTION',
        'GRAFIDA_BTN_IMAGE_URL_INFO', 'GRAFIDA_MSG_IMAGE_URL_PREDICTED_TITLE',
        'GRAFIDA_MSG_IMAGE_URL_PREDICTED', 'GRAFIDA_MSG_IMAGE_URL_PUBLISHED',
        'GRAFIDA_LBL_IMAGE_DECORATIVE', 'GRAFIDA_LBL_IMAGE_CLASS', 'GRAFIDA_BTN_IMAGE_CLASS',
        'GRAFIDA_LBL_MEDIA_BROWSER',
        'GRAFIDA_BTN_CHOOSE_FILE', 'GRAFIDA_BTN_BROWSE_MEDIA', 'GRAFIDA_BTN_CLEAR_IMAGE',
        'GRAFIDA_BTN_MEDIA_UP', 'GRAFIDA_LBL_SOURCE_CODE', 'GRAFIDA_LBL_STYLES',
        'GRAFIDA_LBL_CM_SEARCH', 'GRAFIDA_LBL_CM_REGEXP_HINT', 'GRAFIDA_LBL_CM_WITH',
        'GRAFIDA_LBL_CM_REPLACE_Q', 'GRAFIDA_LBL_CM_REPLACE', 'GRAFIDA_LBL_CM_REPLACE_ALL',
        'GRAFIDA_LBL_CM_REPLACE_WITH', 'GRAFIDA_BTN_CM_ALL', 'GRAFIDA_BTN_CM_STOP',
        'GRAFIDA_LBL_CM_JUMP_TO_LINE', 'GRAFIDA_LBL_CM_JUMP_HINT',
        'GRAFIDA_MSG_NO_IMAGE', 'GRAFIDA_MSG_MEDIA_EMPTY',
        'GRAFIDA_LBL_LOCAL_DRAFTS', 'GRAFIDA_LBL_REMOTE_ARTICLES', 'GRAFIDA_LBL_HAS_LOCAL_DRAFT',
        'GRAFIDA_LBL_ARTICLE_ID', 'GRAFIDA_LBL_ARTICLE_CREATED', 'GRAFIDA_LBL_ARTICLE_MODIFIED',
        'GRAFIDA_MSG_SELECT_SITE', 'GRAFIDA_MSG_LOADING', 'GRAFIDA_MSG_NO_REMOTE_ARTICLES',
        'GRAFIDA_MSG_NO_DRAFTS', 'GRAFIDA_MSG_NO_DRAFTS_YET', 'GRAFIDA_BTN_LIST_SITE_ARTICLES',
        'GRAFIDA_PLACEHOLDER_SEARCH', 'GRAFIDA_LBL_SORT_BY', 'GRAFIDA_LBL_DIRECTION',
        'GRAFIDA_LBL_PER_PAGE', 'GRAFIDA_SORT_DIR_ASC', 'GRAFIDA_SORT_DIR_DESC',
        'GRAFIDA_SORT_ID', 'GRAFIDA_SORT_TITLE', 'GRAFIDA_SORT_CATEGORY', 'GRAFIDA_SORT_ACCESS',
        'GRAFIDA_SORT_AUTHOR', 'GRAFIDA_SORT_LANGUAGE', 'GRAFIDA_SORT_CREATED', 'GRAFIDA_SORT_MODIFIED',
        'GRAFIDA_SORT_PUBLISH_UP', 'GRAFIDA_SORT_PUBLISH_DOWN', 'GRAFIDA_SORT_HITS',
        'GRAFIDA_SORT_FEATURED', 'GRAFIDA_SORT_STATUS', 'GRAFIDA_SORT_ORDERING',
        'GRAFIDA_FILTER_CATEGORY_ANY', 'GRAFIDA_FILTER_TAG_ANY', 'GRAFIDA_FILTER_LANGUAGE_ANY',
        'GRAFIDA_FILTER_STATE_ANY', 'GRAFIDA_FILTER_FEATURED_ANY', 'GRAFIDA_FILTER_FEATURED_YES',
        'GRAFIDA_FILTER_FEATURED_NO', 'GRAFIDA_FILTER_CHECKEDOUT_ANY', 'GRAFIDA_FILTER_CHECKEDOUT_YES',
        'GRAFIDA_FILTER_CHECKEDOUT_NO', 'GRAFIDA_OPT_LANG_ALL', 'GRAFIDA_BTN_CLEAR_FILTERS',
        'GRAFIDA_MSG_FILTERS_RESET',
        'GRAFIDA_BTN_PREV_PAGE', 'GRAFIDA_BTN_NEXT_PAGE', 'GRAFIDA_PAGINATION_INFO',
        'GRAFIDA_NAV_MEDIA', 'GRAFIDA_BTN_UPLOAD', 'GRAFIDA_BTN_NEW_FOLDER', 'GRAFIDA_BTN_RENAME',
        'GRAFIDA_BTN_EDIT_IMAGE', 'GRAFIDA_BTN_RESET_SIZE', 'GRAFIDA_BTN_DOWNLOAD', 'GRAFIDA_BTN_SAVE_TO_DISK',
        'GRAFIDA_BTN_OPEN', 'GRAFIDA_BTN_OPEN_SITE',
        'GRAFIDA_LBL_MEDIA', 'GRAFIDA_LBL_FOLDER', 'GRAFIDA_LBL_FILE',
        'GRAFIDA_LBL_NEW_FOLDER', 'GRAFIDA_LBL_FOLDER_NAME', 'GRAFIDA_LBL_RENAME', 'GRAFIDA_LBL_NEW_NAME',
        'GRAFIDA_TAB_SITE_MEDIA', 'GRAFIDA_TAB_LOCAL_MEDIA', 'GRAFIDA_MSG_LOCAL_MEDIA_EMPTY',
        'GRAFIDA_LBL_MEDIA_USED_BY', 'GRAFIDA_LBL_MEDIA_PUBLISHED', 'GRAFIDA_MSG_MEDIA_SAVED_TO_DISK',
        'GRAFIDA_MSG_DELETE_LOCAL_MEDIA_CONFIRM', 'GRAFIDA_MSG_DELETE_LOCAL_MEDIA_CONFIRM_USED',
        'GRAFIDA_MSG_MEDIA_UPLOADED', 'GRAFIDA_MSG_MEDIA_FOLDER_CREATED', 'GRAFIDA_MSG_MEDIA_RENAMED',
        'GRAFIDA_MSG_MEDIA_DELETED', 'GRAFIDA_MSG_MEDIA_SAVED',
        'GRAFIDA_MSG_DELETE_MEDIA_TITLE', 'GRAFIDA_MSG_DELETE_MEDIA_CONFIRM',
        'GRAFIDA_MSG_DELETE_FOLDER_CONFIRM',
        'GRAFIDA_MSG_MEDIA_OVERWRITE_TITLE', 'GRAFIDA_MSG_MEDIA_OVERWRITE_CONFIRM',
        'GRAFIDA_MSG_MEDIA_EDIT_LOAD_FAIL', 'GRAFIDA_MSG_MEDIA_ONLINE_ONLY',
        'GRAFIDA_LBL_IMAGE_EDITOR', 'GRAFIDA_BTN_ROTATE_LEFT', 'GRAFIDA_BTN_ROTATE_RIGHT',
        'GRAFIDA_BTN_FLIP_H', 'GRAFIDA_BTN_FLIP_V', 'GRAFIDA_BTN_CROP', 'GRAFIDA_BTN_APPLY_CROP',
        'GRAFIDA_BTN_CANCEL_CROP', 'GRAFIDA_BTN_RESIZE', 'GRAFIDA_BTN_RESET',
        'GRAFIDA_LBL_WIDTH', 'GRAFIDA_LBL_HEIGHT', 'GRAFIDA_LBL_LOCK_ASPECT', 'GRAFIDA_LBL_DIMENSIONS',
        'GRAFIDA_MSG_CROP_HINT', 'GRAFIDA_MSG_CROP_ARMED', 'GRAFIDA_LBL_CROP_SELECTION',
        'GRAFIDA_LBL_MEDIA_PREVIEW',
        'GRAFIDA_NAV_AI',
        'GRAFIDA_LBL_AI_SERVICES', 'GRAFIDA_BTN_ADD_AI_SERVICE',
        'GRAFIDA_LBL_AI_PROVIDER', 'GRAFIDA_LBL_AI_ENDPOINT',
        'GRAFIDA_LBL_AI_MODEL', 'GRAFIDA_LBL_AI_KEY', 'GRAFIDA_LBL_AI_PARAMS',
        'GRAFIDA_LBL_DEFAULT_AI_SERVICE', 'GRAFIDA_BTN_SET_DEFAULT',
        'GRAFIDA_MSG_NO_AI_SERVICES',
        'GRAFIDA_MSG_DELETE_AI_SERVICE_CONFIRM',
        'GRAFIDA_MSG_AI_SERVICE_SAVED', 'GRAFIDA_MSG_AI_SERVICE_DELETED',
        'GRAFIDA_MSG_AI_KEY_PLACEHOLDER', 'GRAFIDA_MSG_AI_INSECURE_WARNING',
        'GRAFIDA_LBL_AI_TOOLS', 'GRAFIDA_BTN_ADD_AI_TOOL',
        'GRAFIDA_LBL_AI_TOOL_KEY', 'GRAFIDA_LBL_AI_TOOL_PROMPT',
        'GRAFIDA_LBL_AI_TONE', 'GRAFIDA_LBL_AI_OVERRIDE_SYSTEM',
        'GRAFIDA_LBL_AI_SORT_ORDER', 'GRAFIDA_LBL_AI_SYSTEM_PROMPT',
        'GRAFIDA_BTN_RESTORE_DEFAULT',
        'GRAFIDA_MSG_NO_AI_TOOLS',
        'GRAFIDA_MSG_DELETE_AI_TOOL_CONFIRM',
        'GRAFIDA_MSG_AI_TOOL_SAVED', 'GRAFIDA_MSG_AI_TOOL_DELETED',
        'GRAFIDA_MSG_AI_SYSTEM_PROMPT_SAVED',
        'GRAFIDA_MSG_AI_HOST_MISMATCH',
        'GRAFIDA_LBL_AI_NAME', 'GRAFIDA_LBL_AI_TEMPERATURE', 'GRAFIDA_LBL_AI_TOP_P',
        'GRAFIDA_LBL_AI_MAX_TOKENS', 'GRAFIDA_LBL_AI_STREAM', 'GRAFIDA_BTN_FETCH_MODELS',
        'GRAFIDA_LBL_AI_STORE', 'GRAFIDA_LBL_AI_STORE_DAYS',
        'GRAFIDA_LBL_AI_MULTIMODAL', 'GRAFIDA_MSG_AI_MULTIMODAL_HINT',
        'GRAFIDA_LBL_AI_TOOL_ICON', 'GRAFIDA_BTN_AI_TOOL_ICON_CHOOSE',
        'GRAFIDA_BTN_AI_TOOL_ICON_CLEAR', 'GRAFIDA_PLACEHOLDER_AI_TOOL_ICON_SEARCH',
        'GRAFIDA_MSG_NO_AI_TOOL_ICONS',
        'GRAFIDA_LBL_AI_SERVICE_OVERRIDE',
        'GRAFIDA_OPT_AI_DEFAULT_SERVICE', 'GRAFIDA_OPT_AI_TONE_DEFAULT',
        'GRAFIDA_MSG_AI_DEFAULT_SET', 'GRAFIDA_MSG_AI_MODELS_FAIL', 'GRAFIDA_LBL_AI_CUSTOM_TOOL',
        // AI chat panel (Step 7)
        'GRAFIDA_LBL_AI_CHATS', 'GRAFIDA_LBL_AI_INSTRUCTIONS',
        'GRAFIDA_LBL_AI_THINKING', 'GRAFIDA_LBL_AI_THOUGHT_PROCESS',
        'GRAFIDA_BTN_AI_ASSISTANT', 'GRAFIDA_BTN_AI_TOOLS',
        'GRAFIDA_MENU_AI_CUSTOM_PROMPT',
        'GRAFIDA_BTN_NEW_CHAT',
        'GRAFIDA_BTN_SEND', 'GRAFIDA_BTN_STOP',
        'GRAFIDA_BTN_AI_INSERT', 'GRAFIDA_BTN_COPY',
        'GRAFIDA_PLACEHOLDER_AI_CHAT',
        'GRAFIDA_MSG_AI_NO_SERVICE', 'GRAFIDA_MSG_AI_EMPTY',
        'GRAFIDA_MSG_AI_COPIED', 'GRAFIDA_MSG_AI_COPY_FAIL',
        // Saved chats (Step 8)
        'GRAFIDA_MSG_REMEMBER_CHAT', 'GRAFIDA_MSG_REMEMBER_CHAT_DESC',
        'GRAFIDA_LBL_CHAT_TITLE', 'GRAFIDA_PLACEHOLDER_CHAT_TITLE',
        'GRAFIDA_BTN_REMEMBER', 'GRAFIDA_BTN_DISCARD',
        'GRAFIDA_MSG_CHAT_SAVED', 'GRAFIDA_MSG_CHAT_DELETED', 'GRAFIDA_MSG_CHAT_RENAMED',
        'GRAFIDA_MSG_NO_AI_CHATS', 'GRAFIDA_MSG_DELETE_CHAT_CONFIRM',
        'GRAFIDA_LBL_RENAME_CHAT', 'GRAFIDA_MSG_SAVE_CHAT_CHANGES',
        // Export / import .grafida draft files
        'GRAFIDA_BTN_EXPORT_DRAFT', 'GRAFIDA_BTN_IMPORT_DRAFT', 'GRAFIDA_BTN_REPLACE_DRAFT',
        'GRAFIDA_MSG_DRAFT_EXPORTED', 'GRAFIDA_MSG_DRAFT_IMPORTED', 'GRAFIDA_MSG_DRAFT_REPLACED',
        'GRAFIDA_MSG_REPLACE_DRAFT_TITLE', 'GRAFIDA_MSG_REPLACE_DRAFT_CONFIRM',
        'GRAFIDA_MSG_INVALID_GRAFIDA_FILE',
        // Editor Help dialog (gh-13)
        'GRAFIDA_LBL_HELP_TAB_GRAFIDA', 'GRAFIDA_LBL_HELP_TAB_AI',
        'GRAFIDA_LBL_HELP_ACTION', 'GRAFIDA_LBL_HELP_SHORTCUT',
        'GRAFIDA_LBL_HELP_SC_SAVE', 'GRAFIDA_LBL_HELP_SC_SETTINGS',
        'GRAFIDA_LBL_HELP_SC_CODE', 'GRAFIDA_LBL_HELP_SC_PRE', 'GRAFIDA_LBL_HELP_SC_QUOTE',
        'GRAFIDA_LBL_HELP_SC_PASTE_TEXT',
        'GRAFIDA_MSG_CLIPBOARD_READ_FAIL',
        'GRAFIDA_LBL_HELP_SC_FIND', 'GRAFIDA_LBL_HELP_SC_FIND_NEXT',
        'GRAFIDA_LBL_HELP_SC_FIND_PREV', 'GRAFIDA_LBL_HELP_SC_REPLACE',
        'GRAFIDA_MSG_HELP_AI_INTRO', 'GRAFIDA_MSG_HELP_AI_ASSISTANT', 'GRAFIDA_MSG_HELP_AI_SEND',
        'GRAFIDA_LBL_HELP_AI_TOOLS', 'GRAFIDA_MSG_HELP_AI_TOOLS', 'GRAFIDA_MSG_HELP_AI_CUSTOM',
        'GRAFIDA_MSG_HELP_SLASH_TOOLS',

        // Editor spell checking (gh-24)
        'GRAFIDA_LBL_SPELL_CHECK', 'GRAFIDA_MSG_SPELL_CHECK_INFO',

        // Source editor tag auto-closing (gh-52)
        'GRAFIDA_LBL_AUTO_CLOSE_TAGS', 'GRAFIDA_MSG_AUTO_CLOSE_TAGS_INFO',
        'GRAFIDA_OPT_AUTO_CLOSE_TAGS_FULL', 'GRAFIDA_OPT_AUTO_CLOSE_TAGS_CLOSING',
        'GRAFIDA_OPT_AUTO_CLOSE_TAGS_OFF',

        // Normalising AI-generated content
        'GRAFIDA_LBL_CONTENT_NORMALISATION', 'GRAFIDA_MSG_CONTENT_NORMALISATION_INFO',
        'GRAFIDA_MSG_CONTENT_NORMALISATION_DISCLAIMER',
        'GRAFIDA_OPT_CONTENT_NORMALISATION_FULL', 'GRAFIDA_OPT_CONTENT_NORMALISATION_INVISIBLE',
        'GRAFIDA_OPT_CONTENT_NORMALISATION_OFF',

        // Site metadata cache (gh-42)
        'GRAFIDA_LBL_METADATA_CACHE', 'GRAFIDA_MSG_METADATA_CACHE_INFO', 'GRAFIDA_LBL_METADATA_CACHE_TIME',
        'GRAFIDA_OPT_METADATA_TTL_NEVER', 'GRAFIDA_OPT_METADATA_TTL_15M', 'GRAFIDA_OPT_METADATA_TTL_30M',
        'GRAFIDA_OPT_METADATA_TTL_1H', 'GRAFIDA_OPT_METADATA_TTL_6H', 'GRAFIDA_OPT_METADATA_TTL_12H',
        'GRAFIDA_OPT_METADATA_TTL_1D', 'GRAFIDA_LBL_METADATA_RESET_ON_START',
        'GRAFIDA_MSG_METADATA_RESET_ON_START_INFO',

        // Editor slash commands (gh-9)
        'GRAFIDA_LBL_SLASH_TOOLS', 'GRAFIDA_MSG_SLASH_TOOLS_INFO',
        'GRAFIDA_LBL_SLASH_H1', 'GRAFIDA_LBL_SLASH_H2', 'GRAFIDA_LBL_SLASH_H3',
        'GRAFIDA_LBL_SLASH_BULLET_LIST', 'GRAFIDA_LBL_SLASH_ORDERED_LIST', 'GRAFIDA_LBL_SLASH_LIST_ITEM',
        'GRAFIDA_LBL_SLASH_LOREM_SENTENCE', 'GRAFIDA_LBL_SLASH_LOREM_PARAGRAPH',
        'GRAFIDA_LBL_SLASH_QUOTE', 'GRAFIDA_LBL_SLASH_QUOTATION',
        'GRAFIDA_LBL_SLASH_IMAGE_43', 'GRAFIDA_LBL_SLASH_IMAGE_34', 'GRAFIDA_LBL_SLASH_PLACEHOLDER_ALT',
        'GRAFIDA_LBL_SLASH_FULLSCREEN', 'GRAFIDA_LBL_SLASH_LINK', 'GRAFIDA_LBL_SLASH_TABLE',
        'GRAFIDA_LBL_SLASH_IMAGE', 'GRAFIDA_MSG_SLASH_PLACEHOLDER_FAILED',
        'GRAFIDA_MSG_READMORE_EXISTS',

        // Debug: Diagnose Connection & Request Log (gh-37)
        'GRAFIDA_BTN_DIAGNOSE_CONNECTION', 'GRAFIDA_LBL_API_BASE', 'GRAFIDA_LBL_DIAGNOSE_ATTEMPT',
        'GRAFIDA_LBL_HTTP_REQUEST', 'GRAFIDA_LBL_HTTP_RESPONSE', 'GRAFIDA_LBL_NO_RESPONSE',
        'GRAFIDA_MSG_BINARY_DATA', 'GRAFIDA_MSG_BODY_TRUNCATED',
        'GRAFIDA_LBL_DEBUG', 'GRAFIDA_NAV_REQUEST_LOG', 'GRAFIDA_LBL_REQUEST_LOG',
        'GRAFIDA_MSG_REQUEST_LOG_INFO', 'GRAFIDA_MSG_REQUEST_LOG_DESC', 'GRAFIDA_MSG_NO_REQUESTS',
        'GRAFIDA_LBL_DURATION_MS', 'GRAFIDA_BTN_EXPORT', 'GRAFIDA_BTN_CLEAR',
        'GRAFIDA_MSG_REQUEST_LOG_EXPORTED', 'GRAFIDA_MSG_CLEAR_REQUEST_LOG_CONFIRM',

        // Help screen. Only the chrome is here: the documentation itself is
        // English-only (one source shared with the website) and is served
        // from docs/ by Help\HelpService, not through the language files.
        'GRAFIDA_NAV_HELP', 'GRAFIDA_BTN_HELP', 'GRAFIDA_BTN_HELP_ON_WEB',
        'GRAFIDA_LBL_HELP_FILTER', 'GRAFIDA_MSG_NO_HELP_MATCHES',
        'GRAFIDA_BTN_TOGGLE_CONTENTS', 'GRAFIDA_BTN_RESIZE_CONTENTS',
    ];
}
